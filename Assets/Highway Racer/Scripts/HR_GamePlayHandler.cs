...
public class HR_GamePlayHandler : MonoBehaviour {
	...
	[Tooltip("Optional: Assign a button from the scene to be the Play Button. If empty, one creates automatically.")]
	public GameObject customPlayButton;

	[Header("Spawn Location Of The Cars")]
	public Transform spawnLocation;

	[HideInInspector]public GameObject player;
	private GameObject playButtonRef;
	...
	void Awake(){
		...
		// FORCED SETTINGS FOR USER REQUEST
		mode = Mode.TwoWay;
	...
	void Start () {
		...
		// Initialize MediaPipe Control
		SetupMediaPipe();
	}
	
	private HighwayRacer.MediaPipe.HandLandmarkerRunner _mediaPipeRunner;
	private HighwayRacer.MediaPipe.HandGestureCarController _gestureController;

	void SetupMediaPipe()
	{
		// Create Runner Object
		GameObject mpObj = new GameObject("HandTrackingManager");
		_mediaPipeRunner = mpObj.AddComponent<HighwayRacer.MediaPipe.HandLandmarkerRunner>();
		_gestureController = mpObj.AddComponent<HighwayRacer.MediaPipe.HandGestureCarController>();
		
		// Link
		_mediaPipeRunner.carController = _gestureController;
		
		// Link to Car
		if(player)
		{
			var rcc = player.GetComponent<RCC_CarControllerV3>();
			if(rcc) _gestureController.SetTargetCar(rcc);
		}
	}
	...
	public void CreatePlayButton(){
		...
		if(customPlayButton){
			playButtonRef = customPlayButton;
			playButtonRef.SetActive(true);
			
			Button customBtn = playButtonRef.GetComponent<Button>();
			if(!customBtn) customBtn = playButtonRef.GetComponentInChildren<Button>();
			
			if(customBtn){
				customBtn.onClick.RemoveAllListeners();
				customBtn.onClick.AddListener(OnPlayClicked);
			}
			return;
		}

		GameObject buttonObj = new GameObject("PlayButton");
		buttonObj.transform.SetParent(gameplayCanvas.transform, false);
		playButtonRef = buttonObj;
		...
		Text existingText = gameplayCanvas.GetComponentInChildren<Text>();
		if(existingText)
